...
class DemoFlow:
    ...
    def execute_flow(
        ...
        correlation_id: str | None = None,
        # --- ADDED FOR GUARDRAILS (Amit Bhagat) ---
        # confirmed is set to True by the caller only after the user has
        # answered the confirmation prompt for a sensitive operation.
        ...
        # --- ADDED FOR GUARDRAILS (Amit Bhagat) ---
        # Runs before extraction on purpose. A prompt injection that reaches
        # the extractor has already had its chance to influence the model, and
        # a pasted secret is already in the model's context.
        # -------------------------------------------------------------

        input_decision = guardrail_engine.validate_input(
            normalized_input,
            request_id,
        )
        ...
        # -------------------------------------------------------------
        # Step 2b: Request guardrails
        # --- ADDED FOR GUARDRAILS (Amit Bhagat) ---
        # Scope, identifier format, authorization, privileged accounts, rate
        # limiting and confirmation. Placed before routing so a blocked request
        # never reaches an agent or a tool.
        # -------------------------------------------------------------

        request_decision = guardrail_engine.validate_request(
        ...
        if request_decision.needs_confirmation:
            return self._build_guardrail_response(
                request_id=request_id,
                correlation_id=correlation_id,
                user_input=normalized_input,
        ...
        # --- ADDED FOR GUARDRAILS (Amit Bhagat) ---
        # The unfiltered response is what gets audited; the filtered copy is
        # what the caller receives. A successful reset is recorded here rather
        # than at request time, so blocked or failed attempts do not count
        # against the rate limit.
        # -------------------------------------------------------------

        intent_value = extraction.intent.value

        if (
            intent_value == "password_reset"
            and agent_result.success
        ):
            guardrail_engine.record_reset(
                guardrail_engine.target_identifier(
        ...
        return guardrail_engine.sanitize(response, intent_value)
        # --- END ADDED FOR GUARDRAILS ---
    ...
    def _build_guardrail_response(
        ...
        *,
        ...
        """
        Build the response for a request the guardrails stopped.

        Used for both blocks and confirmation requests, because the shape the
        caller reads is the same either way; only the action differs.

        Args:
            request_id: The request being answered.
            correlation_id: The correlation ID for this conversation.
            user_input: The original query.
            decision: The GuardrailDecision that stopped the request.
            extraction: The extraction result, when the block happened after
                extraction. None when it was an input-stage block.

        Returns:
            A response dict in the same shape as a normal flow response.
        """
        return {
            "success": False,
            "request_id": request_id,
            ...
            "intent": (
                extraction.intent.value if extraction else None
            ),
            "confidence": (
                extraction.confidence if extraction else 0.0
            ),
            "explanation": (
                extraction.explanation if extraction else None
            ),
            "metadata": (
                extraction.metadata.model_dump(mode="json")
                if extraction
            ...
            "clarification_required": False,
            "clarification_question": None,
            # The UI reads these to decide whether to show a block notice or a
            # confirmation prompt.
            ...
            "guardrail_violations": [
                v.model_dump(mode="json") for v in decision.violations
            ],
