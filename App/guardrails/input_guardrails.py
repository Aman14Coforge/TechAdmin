...
def is_allowed_email(value: str) -> bool:
    """
    Decide whether a value is a complete, in-domain email address.

    Comparison is case-insensitive throughout, so Amit.Bhagat@Coforge.com and
    amit.bhagat@coforge.com are treated identically.

    Args:
        value: The candidate identifier.

    Returns:
        True when the value is a well-formed address in an allowed domain.
    """
    if not value:
        return False
...
def check_full_email_present(user_input: str) -> GuardrailDecision:
    """
    Require a complete email address in the raw query.

    Runs before extraction so "reset password for amit.bhagat" is refused in
    milliseconds rather than after a thirty second model call. Both supported
    operations act on one named person, so a query with no address cannot be
    satisfied whatever the model decides.

    Args:
        user_input: The raw user query.

    Returns:
        A GuardrailDecision. Blocks when no in-domain address is present.
    """
    ...
    if ALLOWED_EMAIL_DOMAINS and not any(is_allowed_email(a) for a in addresses):
        return block(
            code=ViolationCode.INVALID_IDENTIFIER,
            rule="Full Email Address Required",
            message=MSG_DOMAIN_NOT_ALLOWED,
            detail=(
                f"No address in an allowed domain "
...
def check_extracted_email(email: str = None) -> GuardrailDecision:
    """
    Require the extracted metadata to carry a complete email address.

    The second half of the same rule. The raw-input check confirms the user
    typed an address; this confirms the model actually extracted one, so a
    query containing an address that the model turned into a bare username is
    still refused rather than being sent to the directory.

    Args:
        email: The email extracted into the metadata, if any.

    Returns:
        A GuardrailDecision. Blocks when the address is absent or out of domain.
    """
    if not REQUIRE_FULL_EMAIL:
        return allow()
