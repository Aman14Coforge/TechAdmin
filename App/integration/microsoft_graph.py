...
class MicrosoftGraphClient:
    ...
    def get_user_details(self, user_identifier: str, select_fields: Optional[list] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Default fields to retrieve
            if select_fields is None:
                # CHANGED FOR MANAGER LOOKUP (Amit Bhagat): jobTitle and department added
                select_fields = ["id", "displayName", "userPrincipalName", "accountEnabled", "onPremisesSyncEnabled", "userType", "mail", "jobTitle", "department"]
    ...
    def get_manager(self, user_identifier: str) -> Dict[str, str]:
        """
        Get the manager's display name and email for a user.

        Microsoft Graph exposes the manager as a navigation property rather
        than a field on the user, so this is a second call:
            GET /users/{id}/manager?$select=displayName,mail

        That is the Graph equivalent of resolving the Active Directory Manager
        distinguished name described in the specification.

        Every failure returns the "Not Available" fallback rather than raising.
        A missing manager must not turn a successful user lookup or password
        reset into an error.

        Args:
            user_identifier: The user's email, UPN or Graph object ID.

        Returns:
            Dict with manager_name and manager_email, each either a real value
            or "Not Available".
        """
        unavailable = {"manager_name": "Not Available", "manager_email": "Not Available"}

        try:
            if not self._ensure_token():
                logger.error("MANAGER_LOOKUP_FAILED | reason=no_access_token")
                return unavailable

            url = (
                f"{self.graph_api_url}/users/{user_identifier}"
                "/manager?$select=displayName,mail,userPrincipalName"
            )

            response = requests.get(url, headers=self._headers(), timeout=10)

            # 404 is the normal answer for "this user has no manager assigned",
            # not an error worth surfacing to the operator.
            if response.status_code == 404:
                logger.info(
                    "MANAGER_LOOKUP_SUCCESS | user={} | manager=none_assigned",
                    user_identifier,
                )
                return unavailable

            if response.status_code != 200:
                self._log_graph_error(response, "Get manager")
                logger.warning(
                    "MANAGER_LOOKUP_FAILED | user={} | status={}",
                    user_identifier,
            ...
            # A manager object can exist without a mail attribute, so fall back
            # to the UPN before giving up on an address.
            manager_email = manager.get("mail") or manager.get("userPrincipalName")

            resolved = {
                "manager_name": manager.get("displayName") or "Not Available",
                "manager_email": manager_email or "Not Available",
            }

            logger.info(
                "MANAGER_LOOKUP_SUCCESS | user={} | manager_resolved={}",
                user_identifier,
                resolved["manager_name"] != "Not Available",
            )
            ...
            logger.error(
                "MANAGER_LOOKUP_FAILED | user={} | error_type={}",
                user_identifier,
                type(exc).__name__,
            )
            return unavailable
    ...
    def find_user_by_username(self, username: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # CHANGED FOR MANAGER LOOKUP (Amit Bhagat): jobTitle and department added
            select_param = "id,displayName,userPrincipalName,accountEnabled,onPremisesSyncEnabled,userType,mail,jobTitle,department"
            escaped = username.replace("'", "''")
            filter_param = (
                f"startswith(userPrincipalName,'{escaped}@') "
                f"or mailNickname eq '{escaped}' "
