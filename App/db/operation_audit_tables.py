...
Both audit tables are written from here:
...
def close_request(request_id: str, response: Dict[str, Any]) -> None:
    """
    Update a request with everything the flow decided.
        ...
        with SessionLocal() as session:
            ...
            if isinstance(result, dict):
                row.target_object_id = (
                    result.get("id") or result.get("user_id") or None
                )
