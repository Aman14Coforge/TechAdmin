...
class GraphAPIPasswordResetTool:
        ...
        try:
            ...
            return self._result(
                ...
                result={
                    ...
                    # the original password is replaced by a masked form. This
                    # class is not currently wired into the MCP servers, which
                    # use HybridPasswordResetTool, but leaving a plaintext
                    # credential in a live file is a leak waiting for the next
                    # person who imports it.
                    "masked_password": mask_password(temporary_password),
                },
            )
        except Exception as exc:
            logger.exception("PASSWORD_RESET_TOOL_FAILED | request_id={} | correlation_id={} | error_type={}", request.request_id, request.correlation_id, type(exc).__name__)
    ...
    def reset_password(self, username: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "user_principal": legacy.get("user_principal"),
            # CHANGED FOR PASSWORD ENHANCEMENTS (Amit Bhagat)
            "masked_password": legacy.get("masked_password"),
            "error": tool_result.error,
        }
